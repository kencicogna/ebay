...
my $connect_string = $opt{P} ? 'DBI:ODBC:BTData_PROD_SQLEXPRESS' : 'DBI:ODBC:BTData_DEV_SQLEXPRESS';
...
my $sql_get_weight = <<END_SQL;
	select ROW_NUMBER() over(order by ebayitemid) as id, *
...
if ( $single_item_id && $weight_in ) {
	...
	$items->{$single_item_id}->{weight} = $weight_in;
}
else {
	# Connect to Database
	eval {
...
for my $item_id ( @all_items ) {
	...
	my $ozs = 0;
	if ( defined $items->{$item_id} && $items->{$item_id} ) {
	  $ozs = $items->{$item_id}->{weight};
	}
	else {
